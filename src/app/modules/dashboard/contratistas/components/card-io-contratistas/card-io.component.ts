...
@Component({
	selector: 'app-card-io-contratistas',
	templateUrl: './card-io.component.html',
	styleUrls: ['./card-io.component.scss']
})
export class CardIoContratistasComponent implements OnInit {
	...
	readEntryAndExits(page: number) {
		this.companie.entrysAndExits({ page: page, rows: 0, download: true }, 'api/Report/SummaryOfVisits').subscribe((response) => {
			this.resumeEntryExits = response.body.list.filter(contratista => contratista.typeUser === "Contratista");
